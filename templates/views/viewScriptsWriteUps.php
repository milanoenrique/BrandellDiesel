                            ...
                            call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data){
								...
								for (var i = 0; i < auxdata.length; i++) {
                                    ...
                                    if(auxdata[i].indexOf('Others:')!=-1){
                                        ...
                                        $("#modal-editwriteup .txtothers").attr("style", "display: initial !important");
                                    }
								}
		...
		$(".fa-volume-up").click(function () {
			...
			$('body, html').on('change','.messageCheckbox',function(){
                ...
                if ($(this).val() == "Others") {
                    ...
                    if ($(this).prop('checked')) {
                        $(".txtothers").attr("style", "display: initial !important");
                    }else{
			...
			$(".buttonSaveWriteUp").click(function(){
				...
				switch(recordtype){
			        ...
			        case 'saveclose':
                             var writeid= '|'+$('#modal-editwriteup input[name="writeid"]').val();
                             var res = writeid.replace("|", "#");
                        var confirm = false;
                         if(window.confirm("Are you sure to save and block the Write-up "+res+" for future modifications?")){
                             URL             = "common/updateWriteUp.php";
					...
					if (checkDate(writeupdate) && checkDate(senioritydate) && checkDate(writeuplastwarn) && checkDate(writeuppreviouswarn)) {
						...
						call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data){
							...
							call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data){
                                    ...
                                    if (DATAW >= "1000" && savecloseflag==true){    
                                        alert("Write-up registered successfully."); 
                                    }
                                    else
                                    {
                                        if (savecloseflag!=false) {
                                            alert("Error trying to save the write up. Please check the user information and try againg.");
                                        }
                                                                        
                                    }

									reloadDashboardWriteUp(VALOR, METODO, 'common/ws_wp.php');
									if (recordtype == 'saveclose' && savecloseflag==true)
                                        {
